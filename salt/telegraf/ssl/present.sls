...
telegraf_crt:
  x509.certificate_managed:
    ...
    - ca_server: {{ ca_server }}
    - signing_policy: telegraf
    - public_key: /etc/pki/telegraf.key
    - CN: {{ HOSTNAME }}
...
{% if grains.role not in ['so-heavynode'] %}
    - unless:
      ...
      - 'enddate=$(date -d "$(openssl x509 -in /etc/pki/telegraf.crt -enddate -noout | cut -d= -f2)" +%s) ; now=$(date +%s) ; expire_date=$(( now + 432000)); [ $enddate -gt $expire_date ]'
{% endif %}
    - timeout: 30
    - retry:
        attempts: 5
